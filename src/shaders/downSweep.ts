...
/* ----------------------------- downSweep ----------------------------------- */
/* ---------------------------------------------------------------------------- */
  downSweep: (numPArg: number, numGArg: number, numGPaddedArg: number) => `#version 450
  ...
  void main() {
    ...
    if (abs(mod(index, 1 << (d + 1))) < epsilon) {
        int t = int(SC.data[index + (1 << d) - 1].scan);
        SC.data[index + (1 << d) - 1].scan = SC.data[index + (1 << (d + 1)) - 1].scan;
        SC.data[index + (1 << (d + 1)) - 1].scan += t;
    }
