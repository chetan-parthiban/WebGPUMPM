export const clearGridDataShader = {
  clearGridData: (numPArg: number, numGArg: number, numGPaddedArg: number) => `#version 450
  ...
  void main() {
    ...
    gridNodes.data[nodeID].m = 0.0;
    gridNodes.data[nodeID].vN = vec3(0.0);
    gridNodes.data[nodeID].v = vec3(0.0);
    gridNodes.data[nodeID].force = vec3(0.0);
    // Test Speed
    // if (nodeID < ${numPArg}) {
