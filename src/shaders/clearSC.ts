...
/* ----------------------------- downSweep ----------------------------------- */
/* ---------------------------------------------------------------------------- */
  clearSC: (numPArg: number, numGArg: number, numGPaddedArg: number) => `#version 450
  ...
  void main() {
    ...
    SC.data[index].criteria = 0;  // Clear criteria buffer
    SC.data[index].scan = 0;  // Clear scan buffer
    SC.data[index].compact = 0;  // Clear compact buffer
    SC.data[index].d = 0;  // Clear d buffer
  }`,
};